...
function tpl_searchform($ajax=true,$autocomplete=true){
  ...
  print '<form action="'.wl().'" class="search" id="dw__search"><div class="no">'; //edit jayjay: charset deleted - handled in init.php
 ...
 * @author Andreas Gohr <andi@splitbrain.org>
 */
function tpl_breadcrumbs($sep='&rarr;'){ //edit jayjay - I like arrows :-)
  global $lang;
  global $conf;
 ...
 */
function tpl_youarehere($sep=' &rarr; '){ //edit jayjay - I like arrows :-)
  global $conf;
  global $ID;
  global $lang;
  ...
  for($i=0; $i<$count - 1; $i++){
    ...
    if($exists){
	  ...
	  $title = ereg_replace('_', ' ', $title); //edit jayjay: remove underscores
...
function tpl_userinfo(){
  ...
  global $INFO;
  if($_SERVER['REMOTE_USER']){
    print $lang['loggedinas'].': '. '<a href="'.XOOPS_URL.'/userinfo.php?uid='.$INFO['userinfo']['uid'].'">'.htmlspecialchars($INFO['userinfo']['name']).'</a>'; //edit jayjay - link to user page
    return true;
  }
  return false;
}
...
/*start edit jayjay - pretty titles without underscores*/
