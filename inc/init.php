 ...
 */
  ...
  if(!defined('DOKU_INC')) define('DOKU_INC',fullpath(dirname(__FILE__).'/../').'/');
  	//start edit jayjay - Xoops wrap
	include_once DOKU_INC."../../mainfile.php";
	unset($lang, $conf);
	define("_CHARSET_BASE", empty($GLOBALS["xlanguage"]['charset_base'])?_CHARSET:$GLOBALS["xlanguage"]['charset_base']);
  	require_once(DOKU_INC.'include/functions.php');
  	//end edit jayjay
  ...
  //start edit jayjay - Xoops will take care of this
  ...
  require_once(DOKU_INC.'inc/lang/'.$conf['lang'].'/lang.php');
	$lang['encoding_locale'] = $lang['encoding'];
	$lang['encoding'] = "UTF-8";
	if(is_object($GLOBALS['xoopsUser'])){
		$_REQUEST['u']=xoops2doku($GLOBALS['xoopsUser']->uname());
	}
	if(!empty($lang['encoding_locale']) && strtolower($lang['encoding_locale']) != "utf-8"){
		foreach(array_keys($lang) as $key){
		  $lang[$key] = XoopsLocal::convert_encoding($lang[$key], $lang['encoding'], $lang['encoding_locale']);
		}
	}
	
	/*
	 * Convert $_POST variables from local encoding charset to dokuwiki encoding charset, which is "utf-8"
	 */
		...
		foreach($_POST as $key=>$val){
			if(is_array($_POST[$key])){
				foreach(array_keys($_POST[$key]) as $_key){
		  			$_POST[$key][$_key] = xoops2doku($_POST[$key][$_key], $lang['encoding'], _CHARSET);
				}
			}else{
		  		$_POST[$key] = XoopsLocal::convert_encoding($_POST[$key], $lang['encoding'], _CHARSET);
			}
	  		$_REQUEST[$key] = $_POST[$key]; 
		}
	}
	/*
	 * Convert $_GET variables submittef by FORM GET method from local encoding charset to dokuwiki encoding charset, which is "utf-8"
	 * 
	 * As I am not able to find a way differ FORM GET method from direct URL method, I have to use the ugly handling
	 */
	if(!empty($_REQUEST['do']) && !empty($_REQUEST['id'])){
		$_REQUEST['id'] = XoopsLocal::convert_encoding($_REQUEST['id'], $lang['encoding'], _CHARSET);
	}
  //end edit jayjay
  ...
  foreach($paths as $c => $p){
    ...
    if(empty($conf[$c]))   {
	    if(is_object($GLOBALS["xoopsUser"]) && $GLOBALS["xoopsUser"]->isAdmin()){
			$GLOBALS["MSG"][]=array('lvl'=>'error', 'msg'=>"<strong>$c</strong> does not exist or isn't writable. Check config!");
		}
    }
    //end edit jayjay
