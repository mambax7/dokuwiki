...
function html_locked(){
  ...
  print '<ul>';
  print '<li><span class="li"><strong>'.$lang['lockedby'].':</strong> '.$INFO['locked'].'</li>'; //edit jayjay - li/div problem in firefox
  print '<li><span class="li"><strong>'.$lang['lockexpire'].':</strong> '.$expire.' ('.$min.' min)</span></li>'; //edit jayjay - li/div problem in firefox
  print '</ul>';
}
...
function html_revisions($first=0){
  ...
  if($INFO['exists'] && $first==0){
    ...
    print '<span class="li">'; //edit jayjay - li/div problem in firefox
    ...
    print ' &ndash; ';
    print $INFO['sum']; //edit jayjay - allow html!
    print ' <span class="user">';
    print (empty($INFO['editor']))?('('.$lang['external_edit'].')'):$INFO['editor']; //edit jayjay - allow html!
    print '</span> ';

    print '('.$lang['current'].')';
    print '</span>'; //edit jayjay - li/div problem in firefox
    print '</li>';
  }
  ...
  foreach($revisions as $rev){
    ...
    print ' <span class="user">';
    if($info['user']){
      print $info['user']; //edit jayjay - allow html!
    }else{
      print $info['ip'];
    }
    print '</span>';
...
function html_recent($first=0){
  ...
  foreach($recents as $recent){
    ...
    print ' <span class="user">';
    if($recent['user']){
      print $recent['user']; //edit jayjay - allow html!
    }else{
      print $recent['ip'];
    }
    print '</span>';
...
function html_buildlist($data,$class,$func,$lifunc='html_li_default'){
  ...
  foreach ($data as $item){
    ...
    $ret .= '<span class="li">';  //edit jayjay - li/div problem in firefox

    $ret .= call_user_func($func,$item);
    $ret .= '</span>';  //edit jayjay - li/div problem in firefox
  }
...
function html_backlinks(){
  ...
  if(!empty($data)) {
      ...
      foreach($data as $blink){
        print '<li><span class="li">'; //edit jayjay - li/div problem in firefox
        print html_wikilink(':'.$blink,$conf['useheading']?NULL:$blink);
        print '</span></li>'; //edit jayjay - li/div problem in firefox
      }
      print '</ul>';
  } else {
...
function html_admin(){
  ...
  foreach ($menu as $item) {
    ...
    ptln('  <li><span class="li"><a href="'.wl($ID, 'do=admin&amp;page='.$item['plugin']).'">'.$item['prompt'].'</a></span></li>'); //edit jayjay - li/div problem in firefox
  }
