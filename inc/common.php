...
function pageinfo(){
  ...
  if($revinfo['user']){
    $info['editor'] = '<a href="'.XOOPS_URL.'/userinfo.php?uid='.$revinfo['user'].'">'.htmlspecialchars(xoops2doku(XoopsUser::getUnameFromId($revinfo['user']))).'</a>'; //edit jayjay - link to user page
  }else{
...
function checklock($id){
  ...
  //start edit jayjay - link to user page
 	if(!empty($ip) && is_numeric($ip)){
		$ip = '<a href="'.XOOPS_URL.'/userinfo.php?uid='.$ip.'">'.htmlspecialchars(xoops2doku(XoopsUser::getUnameFromId($ip))).'</a>';
	}
  //end edit jayjay  
