...
function auth_aclcheck($id,$user,$groups){
  ...
  //start edit jayjay  
  if($GLOBALS["USERINFO"]["perm"] == AUTH_ADMIN){
	  return $GLOBALS["USERINFO"]["perm"];
  }
	// XOOPS MOD
	//--------------------------------------------------------------------  
	if(empty($user)){
		$groups = $GLOBALS["USERINFO"]["grps"];
		$user = $GLOBALS["USERINFO"]["uid"];
	}
	//--------------------------------------------------------------------  
  //end edit jayjay  
  ...
  //start edit jayjay - inherit XOOPS permissions
    $groups[] = '@ALL';
 	if(!empty($user)){
		$groups[] = $user;
	}
  ...
  */
  ...
  //start edit jayjay - force anon auth
      return $perm;
    }elseif($perm > -1) {
      return $perm;
  }
