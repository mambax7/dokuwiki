...
function indexmenu_createThemes(data,indx_list) {
    ...
    var btn,ico,title,key,url,adata,check,f,l,f2,fo2,l2,la,f3,l3;
    adata=data.split(',');
    check=adata.shift();
    if (check != 'indexmenu') {
    ...
    eval("btn.onclick = function(){insertTags('"+
	 jsEscape('wiki__text')+"','"+
	 jsEscape("{{indexmenu_n>")+"','"+
	 jsEscape("}}")+"','"+
	 jsEscape('1')+
	 "');$('picker_plugin_indexmenu').style.display='none';return false;}");
    f3.appendChild(btn);
...
function indexmenu_insertTags(lnk,sep) {
    ...
    insertTags(jsEscape('wiki__text'),jsEscape('[['),jsEscape(']]'),jsEscape(l));
}
